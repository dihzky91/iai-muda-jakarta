...
import { Calendar, MapPin, Clock, Search, ExternalLink, LayoutGrid } from 'lucide-react';
import { CalendarGrid, type CalendarEvent } from './calendar';
...
function toCalendarEvent(e: Event): CalendarEvent {
  return {
    ...
    status: e.status,
    eventType: (e as any).eventType === 'internal' ? 'internal' : 'public',
    color: (e as any).color || 'blue',
    generationId: (e as any).generationId ?? null,
  };
}
  ...
  return (
    <div className="space-y-10 py-8" id="events-section-container">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 max-w-7xl mx-auto bg-white p-4 rounded-3xl border border-slate-100 shadow-sm">
        ...
        {/* Dynamic event search & View Switcher */}
        <div className="flex items-center gap-3 w-full md:w-auto">
          {/* View Mode Switcher Toggle */}
          <div className="flex items-center bg-slate-100/80 p-1 rounded-2xl border border-slate-200/80 shrink-0">
            <button
          ...
          <div className="relative w-full md:w-72">
            ...
            <input
              ...
              className="w-full rounded-xl bg-white border border-slate-200 pl-11 pr-4 py-2 text-xs sm:text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500 transition-all"
            />
          </div>
        </div>
